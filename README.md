...
Авторизация через социальную сеть Вконтакте и Яндекс:

1. Создать суперюзера командой 

```bash
python manage.py createsuperuser
```

2. Для авторизации надо создать приложения. 
- Инструкция для Вконтакте: https://vivazzi.pro/ru/it/auth-vk/
- Инструкция для Яндекс: https://oauth.yandex.ru/client/new
3. Зайти в админ-панель по адресу http://127.0.0.1:8000/admin/ под суперпользователем, созданным в пункте 3.
4. Перейти в Социальные приложения "Social applications" и добавить провайдеров авторизации VK и Yandex. Указать полученные при регистрации приложения client_ID и секретные ключи. Внизу выбрать "Доступные сайты" - перенести в правый список.

По умолчанию приложение доступно по адресу http://127.0.0.1:8000/api/v1/
